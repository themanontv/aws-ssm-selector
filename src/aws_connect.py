...
def preview(selection, instance_data, instance_regex):
    """
...
def main():
    """
    Create a selectable list of EC2 instances in the users aws account
    """
    ...
    for reservation in instance_data:
        for instance in reservation['Instances']:
            ...
            if name == "":
                name = "NONAME"
            items.append(f'{name:<30s} ({instance["InstanceId"]:<15s})')
    ...
    try:
        match = re.search(instance_regex, items[index])
    except TypeError:
        # If type is incorrect likely caused by exiting the menu
    ...
    # Execute the describe instances command
    if is_executable("aws-vault"):
        os.system('aws-vault exec ' + profile + ' -- aws ssm start-session --target ' + match.group(1))
    else:
        os.system('aws ssm start-session --target ' + match.group(1) + ' --profile ' + profile)
